...
public final class ServerRouter {
	...
	public static TableConfig getTableConfig(SchemaConfig schema, String table) {
		if (table == null)
			return null;
		TableConfig tableConfig = schema.getTables().get(table);
		if (tableConfig == null) {
			for (String key : schema.getGroupTables().keySet()) {
				tableConfig = schema.getGroupTables().get(key);
				if (tableConfig.getTableGroupType() == 1) {// 暂时只实现正则
					Pattern p = tableConfig.getTablePattern();
	...
	public static RouteResultset route(SchemaConfig schema, String stmt, String charset, ServerConnection info)
		...
		ft: for (Entry<String, Map<String, List<Object>>> e : astExt.entrySet()) {
			...
			TableConfig tc = getTableConfig(schema, e.getKey());// tables.get(e.getKey());
			if (tc == null) {
				continue;
			}
		...
		info.setTableConfig(matchedTable);
	...
	private static class HintRouter {
		...
		public static void routeFromHint(ServerConnection frontConn, SchemaConfig schema, RouteResultset rrs,
				int prefixIndex, final String sql) throws SQLNonTransientException {
			...
			TableConfig tableConfig = getTableConfig(schema, table);
			SQLStatement ast = null;
			PartitionKeyVisitor visitor = null;
			if (table == null) {// 使用语句中的表名
				ast = SQLParserDelegate.parse(outputSql, MySQLParser.DEFAULT_CHARSET);
				visitor = new PartitionKeyVisitor(schema);
				visitor.setTrimSchema(schema.isKeepSqlSchema() ? schema.getName() : null);
				ast.accept(visitor);
				Map<String, Map<String, List<Object>>> astExt = visitor.getColumnValue();
				for (Entry<String, Map<String, List<Object>>> e : astExt.entrySet()) {
					TableConfig tc = getTableConfig(schema, e.getKey());
					if (tc == null) {
						continue;
					}
					if (tableConfig == null) {
			...
			frontConn.setTableConfig(tableConfig);
			if (table == null || schema.getTables() == null || tableConfig == null) {
				// table not indicated
				RouteResultsetNode[] nodes = new RouteResultsetNode[1];
				rrs.setNodes(nodes);
	...
	private static class MetaRouter {
		...
		private static String getMetaReadDataNode(SchemaConfig schema, String table) {
			...
			Map<String, TableConfig> tables = schema.getTables();
			TableConfig tc = getTableConfig(schema, table);
	...
	public static String genSQL(SQLStatement ast, String orginalSql) {
		StringBuilder s = new StringBuilder();
