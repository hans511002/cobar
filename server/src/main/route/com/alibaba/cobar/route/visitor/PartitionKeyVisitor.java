...
public final class PartitionKeyVisitor implements SQLASTVisitor {
	...
	public PartitionKeyVisitor(SchemaConfig schema) {
		// if (tables == null || tables.isEmpty()) {
	...
	private boolean isRuledColumn(String tableNameUp, String columnNameUp) {
		...
		TableConfig config = ServerRouter.getTableConfig(schema, tableNameUp);
		if (config != null) {
		...
		if (collist != null) {
			for (int i = 0; i < collist.size(); ++i) {
				...
				if (isRuledColumn(tableName, colName)) {
					...
					Map<Object, Set<Pair<Expression, ASTNode>>> valMap = ensureColumnValueIndexObjMap(colValsIndex,
							colName);
					for (RowExpression row : rows) {
