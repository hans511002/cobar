...
public abstract class MySQLDMLParser extends MySQLParser {
	...
	protected void tablePartition(Identifier table) throws SQLSyntaxErrorException {
		if (lexer.token() == MySQLToken.KW_PARTITION) {
			lexer.nextToken();
			if (lexer.token() == MySQLToken.PUNC_LEFT_PAREN) {
				lexer.nextToken();
				Identifier parName = identifier();
				lexer.nextToken();
				table = new Identifier(table.getParent(), table.getIdText() + " PARTITION(" + parName.getIdText() + ")");
			}
		}
	}
