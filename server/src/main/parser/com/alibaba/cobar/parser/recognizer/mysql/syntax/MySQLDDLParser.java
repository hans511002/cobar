...
import com.alibaba.cobar.parser.ast.fragment.ddl.partition.PartitionOption;
...
public class MySQLDDLParser extends MySQLParser {
	public MySQLExprParser exprParser;
	...
	public static enum SpecialIdentifier {
		...
		CHARSET,
		/** EXTENSION syntax */
		POLICY, PARTITION, SUBPARTITION
	}

	public static final Map<String, SpecialIdentifier> specialIdentifiers = new HashMap<String, SpecialIdentifier>(1, 1);
	static {
	...
	private List<IndexOption> indexOptions() throws SQLSyntaxErrorException {
		...
		for (;;) {
			...
			case KW_USING:
				lexer.nextToken();
				IndexOption.IndexType indexType = matchIdentifier("BTREE", "HASH") == 0 ? IndexOption.IndexType.BTREE
	...
	private boolean tableOption(TableOptions options) throws SQLSyntaxErrorException {
		...
		case KW_UNION:
			...
			break os;
		case KW_PARTITION:
			options.setPartiton(new PartitionOption(this, lexer));
			break;
		case IDENTIFIER:
			SpecialIdentifier si = specialIdentifiers.get(lexer.stringValueUppercase());
			if (si != null) {
