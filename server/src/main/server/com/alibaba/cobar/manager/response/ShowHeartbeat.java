		...
		for (String key : dataNodeKeys) {
			...
			if (node != null) {
				...
				for (int i = 0; i < hb.length; i++) {
					...
					if (hb[i] != null) {
						row.add(hb[i].getSource().getConfig().getHost().getBytes());
						row.add(IntegerUtil.toBytes(hb[i].getSource().getConfig().getPort()));
						row.add(IntegerUtil.toBytes(hb[i].getStatus()));
						row.add(IntegerUtil.toBytes(hb[i].getErrorCount()));
						row.add(hb[i].isChecking() ? "checking".getBytes() : "idle".getBytes());
						row.add(LongUtil.toBytes(hb[i].getTimeout()));
						row.add(hb[i].getRecorder().get().getBytes());
						String lat = hb[i].getLastActiveTime();
						row.add(lat == null ? null : lat.getBytes());
						row.add(hb[i].isStop() ? "true".getBytes() : "false".getBytes());
					} else {
						row.add(null);
						row.add(null);
						row.add(null);
