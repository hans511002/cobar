		...
		sc.getProcessor().getExecutor().execute(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if (i == DEFAULT_REPLICA_INDEX) {
						SchemaConfig schema = rrn.getSchema();
						TableConfig table = rrn.getTable();
						int type = rrn.getType();
						int acn = dn.getActiveCount();
						if (schema != null && type != -1 && schema.isOpenRWSep() && acn > 1) {
							int res = -1;
							int tw = schema.getWriteIndex();
							int ac = dn.getActivedIndex();
							if (!dn.getActiveFalg(tw)) {
								tw = ac;
							}
							...
							if (table != null) {
								tw = table.getWriteIndex();
								if (!table.isOpenRWSep() || type != ServerParse.SELECT) {
									if (tw != -1 && dn.getActiveFalg(tw)) {
										res = tw;
									}
								} else {
									if (tw == -1 || !dn.getActiveFalg(tw)) {
										tw = schema.getWriteIndex();
										if (!dn.getActiveFalg(tw)) {
											tw = ac;
										}
									}
							...
							if (type == ServerParse.SELECT) {
								int sel = (int) (dn.getSelCount() % acn);
								if (sel == tw) {
									dn.incSelCount();
									sel += 1;
								}
