...
import com.alibaba.cobar.mysql.bio.executor.MultiNodeComDataExecutor.RowDataPacket;
...
public final class MultiNodeExecutor extends NodeExecutor {
	...
	protected StringBuffer okMessage = new StringBuffer(64);
	protected boolean fieldEOF;
	...
	protected int queryFieldLen;
	protected final DefaultCommitExecutor icExecutor = new DefaultCommitExecutor() {
		@Override
	...
	public boolean decrementCountAndIsZero() {
		final ReentrantLock lock = this.lock;
		lock.lock();
		try {
	...
	public void execute(RouteResultsetNode[] nodes, final boolean autocommit, final BlockingSession ss, final int flag) {
		...
		if (ss.getSource().isClosed()) {
			System.out.println(" getSource client isClosed decrementCountToZero");
	...
	private void execute0(RouteResultsetNode rrn, Channel c, boolean autocommit, BlockingSession ss, int flag) {
		...
		try {
			...
			// 接收和处理数据
			MultiNodeComDataExecutor comPlugn = sc.getComPlugn();
			if (comPlugn != null && comPlugn.getParsed()) {
				comPlugn.readRedirect(this, sc, rrn, c, autocommit, ss, flag, bin);
			} else {
				readRedirect(sc, rrn, c, autocommit, ss, flag, bin);
			}
		} catch (final IOException e) {
			e.printStackTrace();
			c.close();
			System.out.println(" client   run errorerrorerrorerrorerror");
	...
	private void readRedirect(ServerConnection sc, RouteResultsetNode rrn, Channel c, boolean autocommit,
			BlockingSession ss, int flag, BinaryPacket bin) throws IOException {
				...
				if (fieldEOF) {
					...
					for (;;) {
						...
						case EOFPacket.FIELD_COUNT:
							...
							if (sc.IsMultiMeta()) {
								FieldPacket field = null;
								if (headerList.get(1) instanceof FieldPacket) {
									field = (FieldPacket) headerList.get(1);
									field.name = "QUERY_DB_NODE".getBytes();
								} else {
									field = PacketUtil.getField((BinaryPacket) headerList.get(1), "QUERY_DB_NODE");
								}
								field.packetId = ++packetId;
								buffer = field.write(buffer, sc);
								queryFieldLen = headerList.size();
							} else {
								queryFieldLen = headerList.size() - 1;
							}
							headerList = null;
	 ...
	 * 处理RowData数据
	 */
	protected void handleRowData(final RouteResultsetNode rrn, Channel c, BlockingSession ss) throws IOException {
		...
		for (;;) {
			...
			case EOFPacket.FIELD_COUNT:
				...
				return;
			default:
				if (source.IsMultiMeta()) {// 读取数据，组合新的数据记录
					RowDataPacket row = new RowDataPacket(queryFieldLen);
					row.read(bin, queryFieldLen - 1);
					row.packetId = ++packetId;
					row.fieldValues[queryFieldLen - 1] = rrn.getName().getBytes();
					buffer = row.write(buffer, source);
					size += row.packetLength;
				} else {
	...
	public void handleSuccessEOF(BlockingSession ss, BinaryPacket bin) {
		if (decrementCountAndIsZero()) {
			...
			try {
				...
				source.write(bin.write(buffer, source));// 输出到客户端
			} catch (Exception e) {
	...
	public void handleSuccessOK(BlockingSession ss, RouteResultsetNode rrn, boolean autocommit, OkPacket ok) {
		if (ok.message != null && ok.message.length > 0) {
			if (okMessage.length() > 0)
				okMessage.append("\n");
			okMessage.append(rrn.getName());
			okMessage.append(" ");
			okMessage.append(new String(ok.message));
		}
		if (decrementCountAndIsZero()) {
			if (isFail.get()) {
				notifyFailure(ss);
			...
			try {
				...
				if (okMessage.length() > 0)
					ok.message = okMessage.toString().getBytes();
				if (insertId > 0) {
	...
	public void handleFailure(BlockingSession ss, RouteResultsetNode rrn, ErrInfo errInfo) {
		try {
			// 标记为执行失败，并记录第一次异常信息。
			if (!isFail.getAndSet(true) && errInfo != null) {
				errno = errInfo.getErrNo();
				errMessage = "[" + rrn.getName() + "]" + errInfo.getErrMsg();
				errInfo.logErr();
			}
		} catch (Exception e) {
			System.out
					.println("handleFailure failed in " + getClass().getSimpleName() + ", source = " + ss.getSource());
